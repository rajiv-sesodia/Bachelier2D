...
class NeuralNetwork:
    ...
    def calcLoss(self, X, Y, dY):
        ...
        # loss on values
        loss1 = np.sum(np.square(Y-a[self.L-1])) / m
    ...
    def fit(self, optimiser, L2, epochs, X, Y, dY, batchSize, loss, weightsAndBiasesFile='', diagnosticsFile=''):
        ...
        for epoch in range(epochs):
            ...
            # check error
            if epoch % 10 == 0:
                loss1, loss2 = self.calcLoss(X,Y,dY)
        ...
        for n in range(1, self.L):
            for lm1 in range(self.N[n-1]):
                for l in range(self.N[n]):
                    ...
                    # up
                    self.w[n][lm1][l] += eps
                    C_up1, C_up2 = self.calcLoss(X, Y, dY)
                    
                    # down
                    self.w[n][lm1][l] -= 2.0*eps
                    C_dn1, C_dn2 = self.calcLoss(X, Y, dY)
                    
                    # error in deriv
                    dcdw[n][lm1][l] -= (C_up1 - C_dn1) / (2.0 * eps)
